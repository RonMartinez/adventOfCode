...
//	private static final String FILENAME = "src/main/resources/2023/day2InputSample.txt";
	...
	public static void main(String[] args) throws IOException {
		...
		List<Game> games = lines.stream()
				.map(Day2::processGame)
				.collect(Collectors.toList());
		
		games.forEach(t -> System.out.println(ReflectionToStringBuilder.toString(t, ToStringStyle.MULTI_LINE_STYLE)));
		
		List<ColourCount> colourCountLimits = Arrays.asList(
				new ColourCount(Colour.RED, 12L),
				new ColourCount(Colour.GREEN, 13L),
				new ColourCount(Colour.BLUE, 14L)
				);
		
		List<Game> possibleGames = games.stream()
				.filter(pg -> isPossible(pg, colourCountLimits))
				.collect(Collectors.toList());
		
		Long result = possibleGames.stream()
				.map(pg -> pg.getId())
				.reduce(0L, Long::sum);
