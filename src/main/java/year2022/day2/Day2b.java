...
//	private static final String FILENAME = "src/main/resources/2022/day2InputSample.txt";
	...
	private static RpsMatch createRpsMatch(String line) {
		...
		RpsShape rpsShape2 = calculateRpsShape2(encodedShape2, rpsShape1);
	...
	private static RpsShape calculateRpsShape2(String encodedShape, RpsShape opponentRpsShape) {
		RpsShape rpsShape = opponentRpsShape;
		if("X".equals(encodedShape)) {
			rpsShape = RpsShape.PAPER;
			if(RpsShape.ROCK.equals(opponentRpsShape)) {
				rpsShape = RpsShape.SCISSORS;
			} else if(RpsShape.PAPER.equals(opponentRpsShape)) {
				rpsShape = RpsShape.ROCK;
			}
		} else if("Z".equals(encodedShape)) {
			rpsShape = RpsShape.ROCK;
			if(RpsShape.ROCK.equals(opponentRpsShape)) {
				rpsShape = RpsShape.PAPER;
			} else if(RpsShape.PAPER.equals(opponentRpsShape)) {
