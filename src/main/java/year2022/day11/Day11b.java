...
//	private static final String FILENAME = "src/main/resources/2022/day11InputSample.txt";
	...
	public static final Long RELIEF_DIVISOR = 1L;
	public static final Long ITERATIONS = 10_000L;
	...
	public static void main(String[] args) throws IOException {
		...
		monkeyGroup.getMonkeys().stream()
				.forEach(m -> System.out.println(m.getTimesInspected()));
	...
	private static void setReliefOperations(MonkeyGroup monkeyGroup) {
		Long reliefModulo = calculateReliefModulo(monkeyGroup);
		
		ModOperation modOperation = new ModOperation(reliefModulo);
		monkeyGroup.getMonkeys().forEach(m -> m.setReliefOperation(modOperation));
	}

	private static Long calculateReliefModulo(MonkeyGroup monkeyGroup) {
		Long reliefModulo = monkeyGroup.getMonkeys().stream()
				//we know all the tests are DivisibleByTest's
				.map(m -> ((DivisibleByTest)m.getTest()).getModOperation().getValue())
				.reduce(1L, (a,b) -> a*b);
